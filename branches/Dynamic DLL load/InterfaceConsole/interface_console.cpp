...
{
    Interface * poInterface = new InterfaceConsole(exportedMethods);

    return poInterface;
}

InterfaceConsole::InterfaceConsole(struct Interface::stExportedMethods exportedMethods) : meNewPieceType(Piece::NONE), meNewPieceColor(Piece::WHITE)
{
...
{
	DisplayBoard((oGame.*mExportedMethods.pGameOGetBoard)());

    if((oGame.*mExportedMethods.pGameBIsPlayerInCheck)(Piece::WHITE))
    ...
    DisplayCurrentPlayer((oGame.*mExportedMethods.pGameEGetCurrentPlayer)());
}
...
EditionEntry InterfaceConsole::oGetEditionEntry(const GameEdition & oGameEdition)
{
    ...
    DisplayBoard((oGameEdition.*mExportedMethods.pGameEditionOGetBoard)());
...
void InterfaceConsole::DisplayBoard(const Board & oBoard)
{
	...
	for(unsigned int i = 0; i < 8; ++i)
	{
		...
		for(unsigned int j = 0; j < 8; ++j)
		{
			...
			{
                switch((oBoard.*mExportedMethods.pBoardEGetSquareType)(oPos))
				{
				  case Piece::ROOK :
					cPieceChar = 'R';
					break;
                ...
                if((oBoard.*mExportedMethods.pBoardEGetSquareColor)(oPos) == Piece::BLACK)
