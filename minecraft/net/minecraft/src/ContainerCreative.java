    ...
    public ContainerCreative(EntityPlayer entityplayer)
    {
        ...
        for(int k1 = 256; k1 < Item.itemsList.length; k1++)
        {
            if(Item.itemsList[k1] != null)
            {
            	try
            	{
            		ItemStack newItemStack = (ItemStack) Item.itemsList[k1].getStackType().getConstructor(Item.class).newInstance(Item.itemsList[k1]);
            		itemList.add(newItemStack);
            		Item.itemsList[k1].getStackType().getMethod("setCreativeContainerMode", boolean.class).invoke(newItemStack, true);
            	}
            	catch(Exception e)
            	{
            	}
