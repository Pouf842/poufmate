...
{
    Interface * poInterface = new InterfaceConsole(poController);

    return poInterface;
}

InterfaceConsole::InterfaceConsole(Controller * poController)
{
...
void InterfaceConsole::Run()
{
    try
    {
        ...
        {
            DisplayBoard();
            strEntry = strGetEntry();

            /*else if(strEntry[0] == 'P'
               || strEntry[0] == 'p')
               return eGetSelectionPieceEntry(strEntry);*/
            if(strEntry == "c")
                mpoController->CancelLastMove();
            /*else if(strEntry == "r")
            ...
            else
            {
                ...
                {
                    mpoController->GrabPiece(strPosition);
                    moSelectedPosition = strPosition;
                }
                else
                    mpoController->DropPiece(strPosition);
            }
        }
    }
...
void InterfaceConsole::SetMenuState(std::vector<std::string> const * poMenu)
{
    /* Display all the options of the menu */
    for(unsigned int i = 0; i < poMenu->size(); ++i)
    	cout << poMenu->at(i) << endl;
    ...
    while(1)
    {
    	...
    	if(cChoice >= '1' && cChoice <= ('0' + poMenu->size()))
    		mpoController->SetMenuChoice(cChoice - '0');
    	else if(cChoice == 'x')
            mpoController->SetMenuChoice(poMenu->size());	// The last option of the menu is to quit
    }
}

void InterfaceConsole::SetGameState(const Board & oBoard)
{
    moBoard = oBoard;
}

void InterfaceConsole::PieceMoved(Piece * poPiece, const Position & oPos)
{
}

void InterfaceConsole::PieceRemoved(Piece * poPiece)
{
}

void InterfaceConsole::PieceAdded(Piece * poPiece, const Position & oPos)
{
}

void InterfaceConsole::PieceEatPiece(Piece * poEatingPiece, Piece * poAtePiece)
{
}

Piece::PIECE_TYPE InterfaceConsole::eGetPromotionNewPiece()
{
    return Piece::PT_NONE;
}

void InterfaceConsole::Quit()
{
}

void InterfaceConsole::DisplayMessage(std::string strMessage)
{
...
//}
//
void InterfaceConsole::DisplayBoard()
{
    ...
    for(unsigned int i = 0; i < 8; ++i)
    {
	    ...
	    for(unsigned int j = 0; j < 8; ++j)
	    {
		    ...
		    {
                switch(moBoard.eGetSquareType(oPos))
			    {
				    case Piece::PT_ROOK :
				    cPieceChar = 'R';
				    break;
                ...
                if(moBoard.eGetSquareColor(oPos) == Piece::PC_BLACK)
	...
	if(!moSelectedPosition.bIsEmpty())
	{
		int X = moSelectedPosition.mY;
		int Y = moSelectedPosition.mX;
